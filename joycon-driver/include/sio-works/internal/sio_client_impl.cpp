...
#endif
            if (m_sid.size()==0) {
                ss<<uo.get_host()<<":"<<uo.get_port()<<"/8110/socket.io/?EIO=4&transport=websocket&t="<<time(NULL)<<queryString;
            }
            else
            {
                ss<<uo.get_host()<<":"<<uo.get_port()<<"/8110/socket.io/?EIO=4&transport=websocket&sid="<<m_sid<<"&t="<<time(NULL)<<queryString;
            }
            lib::error_code ec;
